...
import paayes
import urllib3
from paayes import six, util

VALID_API_METHODS = ("get", "post", "delete")


class PaayesClientTestCase(object):
    ...
    def request_mocks(self, mocker):
        ...
        for lib in self.REQUEST_LIBRARIES:
            request_mocks[lib] = mocker.patch("paayes.http_client.%s" % (lib,))
        return request_mocks


class TestNewDefaultHttpClient(PaayesClientTestCase):
    @pytest.fixture(autouse=True)
    def setup_warnings(self, request_mocks):
        original_filters = paayes.http_client.warnings.filters[:]
        paayes.http_client.warnings.simplefilter("ignore")
        yield
        paayes.http_client.warnings.filters = original_filters

    def check_default(self, none_libs, expected):
        for lib in none_libs:
            setattr(paayes.http_client, lib, None)

        inst = paayes.http_client.new_default_http_client()

        assert isinstance(inst, expected)

    def test_new_default_http_client_urlfetch(self):
        self.check_default((), paayes.http_client.UrlFetchClient)

    def test_new_default_http_client_requests(self):
        self.check_default(("urlfetch",), paayes.http_client.RequestsClient)

    def test_new_default_http_client_pycurl(self):
        self.check_default(
            ("urlfetch", "requests"), paayes.http_client.PycurlClient
        )
...
class TestRetrySleepTimeDefaultHttpClient(PaayesClientTestCase):
    ...
    def mock_max_delay(self, new_value):
        original_value = paayes.http_client.HTTPClient.MAX_DELAY
        paayes.http_client.HTTPClient.MAX_DELAY = new_value
        try:
            yield self
        finally:
            paayes.http_client.HTTPClient.MAX_DELAY = original_value

    def test_sleep_time_exponential_back_off(self):
        client = paayes.http_client.new_default_http_client()
    ...
    def test_initial_delay_as_minimum(self):
        ...
        initial_delay = paayes.http_client.HTTPClient.INITIAL_DELAY
        self.assert_sleep_times(client, [initial_delay] * 5)

    def test_maximum_delay(self):
        client = paayes.http_client.new_default_http_client()
        client._add_jitter_time = lambda t: t
        max_delay = paayes.http_client.HTTPClient.MAX_DELAY
    ...
    def test_randomness_added(self):
        ...
        base_value = paayes.http_client.HTTPClient.INITIAL_DELAY * random_value

        with self.mock_max_delay(10):
            expected = [
                paayes.http_client.HTTPClient.INITIAL_DELAY,
                base_value * 2,
                base_value * 4,
                base_value * 8,
                base_value * 16,
...
class TestRetryConditionsDefaultHttpClient(PaayesClientTestCase):
    ...
    def test_should_retry_on_paayes_should_retry_true(self, mocker):
        client = paayes.http_client.new_default_http_client()
        client._max_network_retries = lambda: 1
        headers = {"paayes-should-retry": "true"}
    ...
    def test_should_retry_on_paayes_should_retry_false(self, mocker):
        client = paayes.http_client.new_default_http_client()
        client._max_network_retries = lambda: 1
        headers = {"paayes-should-retry": "false"}
...
class TestHTTPClient(object):
    ...
    def setup_paayes(self):
        orig_attrs = {"enable_telemetry": paayes.enable_telemetry}
    ...
    def test_sends_telemetry_on_second_request(self, mocker):
        class TestClient(paayes.http_client.HTTPClient):
            pass
        ...
        assert "X-Paayes-Client-Telemetry" in args[2]

        telemetry = json.loads(args[2]["X-Paayes-Client-Telemetry"])
...
class ClientTestBase(object):
    ...
    def valid_url(self, path="/foo"):
        return "https://api.paayes.com%s" % (path,)
    ...
    def test_exception(self, request_mock, mock_error):
        ...
        with pytest.raises(paayes.error.APIConnectionError):
            self.make_request("get", self.valid_url, {}, None)


class RequestsVerify(object):
    def __eq__(self, other):
        return other and other.endswith("paayes/data/ca-certificates.crt")


class TestRequestsClient(PaayesClientTestCase, ClientTestBase):
    REQUEST_CLIENT = paayes.http_client.RequestsClient
...
class TestRequestClientRetryBehavior(TestRequestsClient):
    ...
    def connection_error(self, session):
        ...
        def connection_error(given_exception):
            with pytest.raises(paayes.error.APIConnectionError) as error:
    ...
    def test_handle_request_error_should_not_retry(
        ...
        assert "not verify Paayes's SSL certificate" in error.user_message
...
class TestUrlFetchClient(PaayesClientTestCase, ClientTestBase):
    REQUEST_CLIENT = paayes.http_client.UrlFetchClient
...
class TestUrllib2Client(PaayesClientTestCase, ClientTestBase):
    REQUEST_CLIENT = paayes.http_client.Urllib2Client
...
class TestPycurlClient(PaayesClientTestCase, ClientTestBase):
    REQUEST_CLIENT = paayes.http_client.PycurlClient
    ...
    def bio_mock(self, mocker):
        bio_patcher = mocker.patch("paayes.util.io.BytesIO")
    ...
    def mock_error(self):
        def mock_error(mock):
            ...
            paayes.http_client.pycurl.error = FakeException
            mock.perform.side_effect = paayes.http_client.pycurl.error
    ...
    def check_call(self, request_mocks):
        def check_call(
            ...
            mock.setopt.assert_any_call(lib_mock.URL, paayes.util.utf8(url))
...
class TestAPIEncode(PaayesClientTestCase):
    def test_encode_dict(self):
        body = {"foo": {"dob": {"month": 1}, "name": "bat"}}

        values = [t for t in paayes.api_requestor._api_encode(body)]
