...
class TestListObject(object):
    ...
    def list_object(self):
        return paayes.ListObject.construct_from(
    ...
    def test_list(self, request_mock, list_object):
        ...
        res = list_object.list(myparam="you", paayes_account="acct_123")

        request_mock.assert_requested(
            "get", "/my/path", {"myparam": "you"}, None
        )
        assert isinstance(res, paayes.ListObject)
        assert res.paayes_account == "acct_123"
        assert isinstance(res.data, list)
        assert isinstance(res.data[0], paayes.Charge)
    ...
    def test_create(self, request_mock, list_object):
        ...
        res = list_object.create(myparam="eter", paayes_account="acct_123")
    ...
    def test_create_maintains_list_properties(self, request_mock, list_object):
        ...
        # test otherwise
        charge = paayes.Charge.retrieve("ch_123", api_key="sk_test_custom")
        res = charge.refunds.create(amount=123)
        request_mock.assert_requested(
            "post", "/api/v1/charges/ch_123/refunds", {"amount": 123}, None
        )
    ...
    def test_retrieve(self, request_mock, list_object):
        ...
        res = list_object.retrieve(
            "myid", myparam="cow", paayes_account="acct_123"
        )
    ...
    def test_is_empty(self):
        lo = paayes.ListObject.construct_from({"data": []}, None)
        assert lo.is_empty is True

    def test_empty_list(self):
        lo = paayes.ListObject.empty_list()
        assert lo.is_empty

    def test_iter(self):
        arr = [{"id": 1}, {"id": 2}, {"id": 3}]
        expected = paayes.util.convert_to_paayes_object(arr)
        lo = paayes.ListObject.construct_from({"data": arr}, None)
        assert list(lo) == expected

    def test_iter_reversed(self):
        arr = [{"id": 1}, {"id": 2}, {"id": 3}]
        expected = paayes.util.convert_to_paayes_object(list(reversed(arr)))
        lo = paayes.ListObject.construct_from({"data": arr}, None)
    ...
    def test_next_page_empty_list(self):
        ...
        next_lo = lo.next_page()
        assert next_lo == paayes.ListObject.empty_list()
    ...
    def test_serialize_empty_list(self):
        ...
        deserialized = paayes.ListObject.construct_from(
    ...
    def test_serialize_nested_empty_list(self):
        ...
        obj = paayes.paayes_object.PaayesObject.construct_from(
            {"nested": empty}, "mykey"
        )
        serialized = str(obj)
        deserialized = paayes.paayes_object.PaayesObject.construct_from(
            json.loads(serialized), "mykey"
        )
...
class TestAutoPaging:
    ...
    def test_class_method_two_pages(self, request_mock):
        ...
        seen = [
            item["id"]
            for item in paayes.Charge.auto_paging_iter(limit=25, foo="bar")
        ]

        request_mock.assert_requested(
            "get", "/api/v1/charges", {"limit": 25, "foo": "bar"}
        )
        assert seen == ["ch_001"]
