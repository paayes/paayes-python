...
class TestCard(object):
    def construct_resource(self, **params):
        ...
        return paayes.Card.construct_from(card_dict, paayes.api_key)

    def test_has_account_instance_url(self):
        resource = self.construct_resource(account="acct_123")
        assert (
            resource.instance_url()
            == "/api/v1/accounts/acct_123/external_accounts/%s" % TEST_RESOURCE_ID
        )
    ...
    def test_has_recipient_instance_url(self):
        ...
        assert (
            resource.instance_url()
            == "/api/v1/recipients/rp_123/cards/%s" % TEST_RESOURCE_ID
        )
    ...
    def test_is_not_retrievable(self):
        with pytest.raises(NotImplementedError):
            paayes.Card.retrieve(TEST_RESOURCE_ID)
    ...
    def test_is_not_modifiable(self):
        with pytest.raises(NotImplementedError):
            paayes.Card.modify(TEST_RESOURCE_ID, metadata={"key": "value"})
