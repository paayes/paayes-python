...
class TestAPIResource(object):
    ...
    def test_convert_to_paayes_object(self):
        sample = {
            "foo": "bar",
        ...
        converted = paayes.util.convert_to_paayes_object(
            sample, "akey", None, None
        )

        # Types
        assert isinstance(converted, paayes.paayes_object.PaayesObject)
        assert isinstance(converted.adict, paayes.Charge)
        assert len(converted.alist) == 1
        assert isinstance(converted.alist[0], paayes.Customer)
