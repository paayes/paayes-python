...
from paayes.paayes_response import PaayesResponse, PaayesStreamResponse

from paayes.six.moves.urllib.parse import urlsplit
...
class APIHeaderMatcher(object):
    ...
    def __init__(
        ...
        self.api_key = api_key or paayes.api_key
        self.extra = extra
    ...
    def __eq__(self, other):
        return (
            ...
            and self._x_paayes_ua_contains_app_info(other)
    ...
    def _x_paayes_ua_contains_app_info(self, other):
        if self.app_info:
            ua = json.loads(other["X-Paayes-Client-User-Agent"])
...
class QueryMatcher(object):
    ...
    def __eq__(self, other):
        ...
        parsed = paayes.util.parse_qsl(query)
...
class UrlMatcher(object):
    ...
    def __eq__(self, other):
        ...
        q_matcher = QueryMatcher(paayes.util.parse_qsl(self.exp_parts.query))
...
class TestAPIRequestor(object):
    ...
    ENCODE_EXPECTATIONS = {
        ...
        "string": [("%s", "boo")],
        "unicode": [("%s", paayes.util.utf8(u"\u1234"))],
    ...
    def http_client(self, mocker):
        http_client = mocker.Mock(paayes.http_client.HTTPClient)
    ...
    def requestor(self, http_client):
        requestor = paayes.api_requestor.APIRequestor(client=http_client)
        return requestor
    ...
    def check_call(self, http_client):
        def check_call(
            ...
            if not abs_url:
                abs_url = "%s%s" % (paayes.api_base, self.valid_path)
            if not headers:
    ...
    def encoder_check(self, key):
        ...
        stk = []
        fn = getattr(paayes.api_requestor.APIRequestor, "encode_%s" % (key,))
    ...
    def _test_encode_naive_datetime(self):
        ...
        paayes.api_requestor.APIRequestor.encode_datetime(
    ...
    def test_dictionary_list_encoding(self):
        ...
        encoded = list(paayes.api_requestor._api_encode(params))
        key, value = encoded[0]
    ...
    def test_url_construction(self, requestor, mock_response, check_call):
        CASES = (
            ("%s?foo=bar" % paayes.api_base, "", {"foo": "bar"}),
            ("%s?foo=bar" % paayes.api_base, "?", {"foo": "bar"}),
            (paayes.api_base, "", {}),
            (
                "%s/%%20spaced?foo=bar%%24&baz=5" % paayes.api_base,
                "/%20spaced?foo=bar%24",
                {"baz": "5"},
            ),
            (
                "%s?foo=bar&foo=bar" % paayes.api_base,
                "?foo=bar",
                {"foo": "bar"},
            ),
        )
    ...
    def test_empty_methods(self, requestor, mock_response, check_call):
        for meth in VALID_API_METHODS:
            ...
            assert isinstance(resp, PaayesResponse)
    ...
    def test_empty_methods_streaming_response(
        ...
        for meth in VALID_API_METHODS:
            ...
            assert isinstance(resp, PaayesStreamResponse)
    ...
    def test_methods_with_params_and_response(
        ...
        for method in VALID_API_METHODS:
            ...
            if method == "post":
                check_call(
                    method,
                    post_data=QueryMatcher(paayes.util.parse_qsl(encoded)),
                )
            else:
                abs_url = "%s%s?%s" % (
                    paayes.api_base,
    ...
    def test_uses_instance_key(self, http_client, mock_response, check_call):
        key = "fookey"
        requestor = paayes.api_requestor.APIRequestor(key, client=http_client)
    ...
    def test_uses_instance_api_version(
        ...
        requestor = paayes.api_requestor.APIRequestor(
        ...
        check_call(
            ...
            headers=APIHeaderMatcher(
                extra={"Paayes-Version": "fooversion"}, request_method="get"
            ),
        )
    ...
    def test_uses_instance_account(
        ...
        check_call(
            "get",
            headers=APIHeaderMatcher(
                extra={"Paayes-Account": account}, request_method="get"
            ),
        )

    def test_sets_default_http_client(self, http_client):
        assert not paayes.default_http_client

        paayes.api_requestor.APIRequestor(client=http_client)

        # default_http_client is not populated if a client is provided
        assert not paayes.default_http_client

        paayes.api_requestor.APIRequestor()

        # default_http_client is set when no client is specified
        assert paayes.default_http_client

        new_default_client = paayes.default_http_client
        paayes.api_requestor.APIRequestor()

        # the newly created client is reused
        assert paayes.default_http_client == new_default_client
    ...
    def test_uses_app_info(self, requestor, mock_response, check_call):
        try:
            ...
            ua = "Paayes/v1 PythonBindings/%s" % (paayes.version.VERSION,)
    ...
    def test_fails_without_api_key(self, requestor):
        ...
        with pytest.raises(paayes.error.AuthenticationError):
            requestor.request("get", self.valid_path, {})

    def test_invalid_request_error_404(self, requestor, mock_response):
        mock_response('{"error": {}}', 404)

        with pytest.raises(paayes.error.InvalidRequestError):
            requestor.request("get", self.valid_path, {})
    ...
    def test_idempotency_error(self, requestor, mock_response):
        ...
        with pytest.raises(paayes.error.IdempotencyError):
            requestor.request("get", self.valid_path, {})
    ...
    def test_permissions_error(self, requestor, mock_response):
        ...
        with pytest.raises(paayes.error.PermissionError):
            requestor.request("get", self.valid_path, {})

    def test_card_error(self, requestor, mock_response):
        mock_response('{"error": {"code": "invalid_expiry_year"}}', 402)

        with pytest.raises(paayes.error.CardError) as excinfo:
            requestor.request("get", self.valid_path, {})
    ...
    def test_rate_limit_error(self, requestor, mock_response):
        ...
        with pytest.raises(paayes.error.RateLimitError):
            requestor.request("get", self.valid_path, {})
    ...
    def test_server_error(self, requestor, mock_response):
        ...
        with pytest.raises(paayes.error.APIError):
            requestor.request("get", self.valid_path, {})
    ...
    def test_raw_request_with_file_param(self, requestor, mock_response):
        ...
        mock_response("{}", 200)
        requestor.request("post", "/api/v1/files", params, supplied_headers)
...
class TestDefaultClient(object):
    ...
    def test_default_http_client_called(self, mocker):
        hc = mocker.Mock(paayes.http_client.HTTPClient)
        hc._verify_ssl_certs = True
        hc.name = "mockclient"
        hc.request_with_retries = mocker.Mock(return_value=("{}", 200, {}))

        paayes.default_http_client = hc
        paayes.Charge.list(limit=3)

        hc.request_with_retries.assert_called_with(
            "get",
            "https://api.paayes.com/api/v1/charges?limit=3",
            mocker.ANY,
            None,
        )
