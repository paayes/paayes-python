...
from paayes.six.moves import builtins
...
class TestUtil(object):
    ...
    def test_test_apikey(self, mocker):
        mocker.patch("paayes.api_key", "sk_test_KOWobxXidxNlIx")
        link = util.dashboard_link(self.DUMMY_REQ_ID)
        assert (
            link
            == "https://dashboard.paayes.com/test/logs/" + self.DUMMY_REQ_ID
        )

    def test_live_apikey(self, mocker):
        mocker.patch("paayes.api_key", "sk_live_axwITqZSgTUXSN")
        link = util.dashboard_link(self.DUMMY_REQ_ID)
        assert (
            link
            == "https://dashboard.paayes.com/live/logs/" + self.DUMMY_REQ_ID
        )

    def test_no_apikey(self, mocker):
        mocker.patch("paayes.api_key", None)
        link = util.dashboard_link(self.DUMMY_REQ_ID)
        assert (
            link
            == "https://dashboard.paayes.com/test/logs/" + self.DUMMY_REQ_ID
    ...
    def test_old_apikey(self, mocker):
        mocker.patch("paayes.api_key", "axwITqZSgTUXSN")
        link = util.dashboard_link(self.DUMMY_REQ_ID)
        assert (
            link
            == "https://dashboard.paayes.com/test/logs/" + self.DUMMY_REQ_ID
    ...
    def log_test_loop(self, test_cases, logging_func, logger_name, mocker):
        for case in test_cases:
            try:
                ...
                mocker.patch("paayes.log", case.flag)
                mocker.patch("paayes.util.PAAYES_LOG", case.env)
    ...
    def test_log_debug(self, mocker):
        # (PAAYES_LOG, paayes.log): should_output?
        test_cases = [
        ...
        self.log_test_loop(
            ...
            logger_name="paayes.util.logger.debug",
            mocker=mocker,
        )
    ...
    def test_log_info(self, mocker):
        ...
        self.log_test_loop(
            ...
            logger_name="paayes.util.logger.info",
            mocker=mocker,
        )
    ...
    def test_convert_to_paayes_object_and_back(self):
        resp = {
            "object": "balance",
            "available": [
                {
        ...
        obj = util.convert_to_paayes_object(resp)
        assert isinstance(obj, paayes.Balance)
        assert type(obj.available) == list
        assert isinstance(obj.available[0], paayes.paayes_object.PaayesObject)
