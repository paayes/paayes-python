...
class TestPaayesObject(object):
    def test_initializes_with_parameters(self):
        obj = paayes.paayes_object.PaayesObject(
    ...
    def test_access(self):
        obj = paayes.paayes_object.PaayesObject("myid", "mykey", myparam=5)
    ...
    def test_refresh_from(self, mocker):
        ...
        assert obj.foo == "bar"
        assert obj["trans"] == "me"
        assert obj.paayes_version is None
        assert obj.paayes_account is None
        ...
        assert obj.paayes_version == "2017-08-15"
        assert obj.paayes_account == "acct_foo"
    ...
    def test_passing_nested_refresh(self):
        ...
        assert nested.paayes_account == "acct_foo"
    ...
    def test_refresh_from_nested_object(self):
        ...
        assert isinstance(
            obj.lines.subscriptions[0], paayes.paayes_object.PaayesObject
        )
    ...
    def test_repr(self):
        obj = paayes.paayes_object.PaayesObject("foo", "bar", myparam=5)
        ...
        assert u"<PaayesObject \u4e00boo\u1f00" in res
    ...
    def test_deletion(self):
        obj = paayes.paayes_object.PaayesObject("id", "key")
    ...
    def test_copy(self):
        nested = paayes.paayes_object.PaayesObject.construct_from(
            {"value": "bar"}, "mykey"
        )
        obj = paayes.paayes_object.PaayesObject.construct_from(
            {"empty": "", "value": "foo", "nested": nested},
            "mykey",
            paayes_account="myaccount",
        )
        ...
        assert copied.paayes_account == "myaccount"
    ...
    def test_to_dict_recursive(self):
        foo = paayes.paayes_object.PaayesObject.construct_from(
            {"value": "foo"}, "mykey"
        )
        bar = paayes.paayes_object.PaayesObject.construct_from(
            {"value": "bar"}, "mykey"
        )
        obj = paayes.paayes_object.PaayesObject.construct_from(
        ...
        assert not isinstance(
            d["nested"][0], paayes.paayes_object.PaayesObject
        )
        assert not isinstance(
            d["nested"][1], paayes.paayes_object.PaayesObject
        )

    def test_serialize_empty_string_unsets(self):
        class SerializeToEmptyString(paayes.paayes_object.PaayesObject):
