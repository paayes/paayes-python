...
from paayes import six, error


class TestPaayesError(object):
    def test_formatting(self):
        err = error.PaayesError(u"öre")
    ...
    def test_formatting_with_request_id(self):
        err = error.PaayesError(u"öre", headers={"request-id": "123"})
    ...
    def test_formatting_with_none(self):
        err = error.PaayesError(None, headers={"request-id": "123"})
    ...
    def test_repr(self):
        ...
        if six.PY2:
            assert (
                repr(err)
                == "PaayesError(message=u'\\xf6re', http_status=None, "
                "request_id='123')"
            )
        else:
            assert (
                repr(err) == "PaayesError(message='öre', http_status=None, "
                "request_id='123')"
            )

    def test_error_object(self):
        err = error.PaayesError(
    ...
    def test_error_object_not_dict(self):
        err = error.PaayesError("message", json_body={"error": "not a dict"})
        assert err.error is None


class TestPaayesErrorWithParamCode(object):
    def test_repr(self):
        err = error.CardError(
            u"öre",
            param="cparam",
