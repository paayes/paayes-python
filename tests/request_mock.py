...
class RequestMock(object):
    def __init__(self, mocker):
        ...
        self._real_request = paayes.api_requestor.APIRequestor.request
        self._real_request_stream = (
            paayes.api_requestor.APIRequestor.request_stream
        )
        self._stub_request_handler = StubRequestHandler()

        self.constructor_patcher = self._mocker.patch(
            "paayes.api_requestor.APIRequestor.__init__",
            side_effect=paayes.api_requestor.APIRequestor.__init__,
            autospec=True,
        )

        self.request_patcher = self._mocker.patch(
            "paayes.api_requestor.APIRequestor.request",
    ...
    def _patched_request(self, requestor, method, url, *args, **kwargs):
        ...
        if response is not None:
            return response, paayes.api_key
...
class StubRequestHandler(object):
    ...
    def get_response(self, method, url, expect_stream=False):
        if (method, url) in self._entries:
            ...
            if is_streaming:
                paayes_response = PaayesStreamResponse(
                    util.io.BytesIO(str.encode(rbody)), rcode, rheaders
                )
            else:
                paayes_response = PaayesResponse(rbody, rcode, rheaders)
            return paayes_response
