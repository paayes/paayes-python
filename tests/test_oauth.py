...
from paayes.six.moves.urllib.parse import parse_qs, urlparse
...
class TestOAuth(object):
    def test_authorize_url(self, request_mock):
        url = paayes.OAuth.authorize_url(
            scope="read_write",
            state="csrf_token",
            paayes_user={
        ...
        assert params["paayes_user[email]"] == ["test@example.com"]
        assert params["paayes_user[url]"] == [
            "https://example.com/profile/test"
        ]
        assert params["paayes_user[country]"] == ["US"]
    ...
    def test_deauthorize(self, request_mock):
        ...
        resp = paayes.OAuth.deauthorize(paayes_user_id="acct_test_deauth")
        request_mock.assert_requested(
            "post",
            "/oauth/deauthorize",
            {"client_id": "ca_123", "paayes_user_id": "acct_test_deauth"},
        )
        assert resp["paayes_user_id"] == "acct_test_deauth"
