...
import paayes
from paayes import error, oauth_error, http_client, version, util, six
from paayes.multipart_data_generator import MultipartDataGenerator
from paayes.six.moves.urllib.parse import urlencode, urlsplit, urlunsplit
...
def _api_encode(data):
    for key, value in six.iteritems(data):
        ...
        if value is None:
            continue
        elif hasattr(value, "paayes_id"):
            yield (key, value.paayes_id)
...
class APIRequestor(object):
    def __init__(
        ...
        self.api_base = api_base or paayes.api_base
        self.api_key = key
        self.api_version = api_version or paayes.api_version
        self.paayes_account = account

        self._default_proxy = None

        from paayes import verify_ssl_certs as verify
        from paayes import proxy

        if client:
            self._client = client
        elif paayes.default_http_client:
            self._client = paayes.default_http_client
            if proxy != self._default_proxy:
                warnings.warn(
                    "paayes.proxy was updated after sending a "
                    "request - this is a no-op. To use a different proxy, "
                    "set paayes.default_http_client to a new client "
                    "configured with the proxy."
                )
        else:
            # If the paayes.default_http_client has not been set by the user
            # yet, we'll set it here. This way, we aren't creating a new
            # HttpClient for every request.
            paayes.default_http_client = http_client.new_default_http_client(
    ...
    def request_headers(self, api_key, method):
        user_agent = "Paayes/v1 PythonBindings/%s" % (version.VERSION,)
        if paayes.app_info:
            user_agent += " " + self.format_app_info(paayes.app_info)
        ...
        for attr, func in [
            ...
            ua[attr] = val
        if paayes.app_info:
            ua["application"] = paayes.app_info

        headers = {
            "X-Paayes-Client-User-Agent": json.dumps(ua),
            "User-Agent": user_agent,
            "Authorization": "Bearer %s" % (api_key,),
        }

        if self.paayes_account:
            headers["Paayes-Account"] = self.paayes_account
        ...
        if self.api_version is not None:
            headers["Paayes-Version"] = self.api_version
    ...
    def request_raw(
        ...
        if my_api_key is None:
            raise error.AuthenticationError(
                ...
                '"paayes.api_key = <API-KEY>"). You can generate API keys '
                "from the Paayes web interface.  See https://account.paayes.com/api/key "
                "for details, or email support@paayes.com if you have any "
                "questions."
            )
        ...
        if method == "get" or method == "delete":
            ...
            raise error.APIConnectionError(
                ...
                "Paayes bindings.  Please contact support@paayes.com for "
                "assistance." % (method,)
            )
        ...
        util.log_info("Request to Paayes api", method=method, path=abs_url)
        util.log_debug(
            "Post details",
            post_data=encoded_params,
        ...
        util.log_info("Paayes API response", path=abs_url, response_code=rcode)
    ...
    def interpret_response(self, rbody, rcode, rheaders):
        try:
            ...
            resp = PaayesResponse(rbody, rcode, rheaders)
        except Exception:
            raise error.APIError(
    ...
    def interpret_streaming_response(self, stream, rcode, rheaders):
        ...
        if self._should_handle_code_as_error(rcode):
            ...
            return PaayesStreamResponse(stream, rcode, rheaders)
