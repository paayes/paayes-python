...
import paayes
from paayes import api_requestor, util, six
...
class PaayesObject(dict):
    ...
    def __init__(
        ...
        **params
        ...
        super(PaayesObject, self).__init__()
        ...
        object.__setattr__(self, "paayes_version", paayes_version)
        object.__setattr__(self, "paayes_account", paayes_account)
    ...
    def update(self, update_dict):
        ...
        return super(PaayesObject, self).update(update_dict)

    def __setattr__(self, k, v):
        if k[0] == "_" or k in self.__dict__:
            return super(PaayesObject, self).__setattr__(k, v)
    ...
    def __delattr__(self, k):
        if k[0] == "_" or k in self.__dict__:
            return super(PaayesObject, self).__delattr__(k)
        else:
            del self[k]
    ...
    def __setitem__(self, k, v):
        ...
        super(PaayesObject, self).__setitem__(k, v)

    def __getitem__(self, k):
        try:
            return super(PaayesObject, self).__getitem__(k)
        except KeyError as err:
            if k in self._transient_values:
                raise KeyError(
                    "%r.  HINT: The %r attribute was set in the past."
                    "It was then wiped when refreshing the object with "
                    "the result returned by Paayes's API, probably as a "
    ...
    def __delitem__(self, k):
        super(PaayesObject, self).__delitem__(k)
    ...
    def refresh_from(
        ...
        self.paayes_version = paayes_version or getattr(
            values, "paayes_version", None
        )
        self.paayes_account = paayes_account or getattr(
            values, "paayes_account", None
        )
        ...
        for k, v in six.iteritems(values):
            super(PaayesObject, self).__setitem__(
                k,
                util.convert_to_paayes_object(
                    v, api_key, paayes_version, paayes_account
                ),
            )
    ...
    def request(self, method, url, params=None, headers=None):
        ...
        return util.convert_to_paayes_object(
            response, api_key, self.paayes_version, self.paayes_account
        )
    ...
    def to_dict_recursive(self):
        def maybe_to_dict_recursive(value):
            if value is None:
                return None
            elif isinstance(value, PaayesObject):
    ...
    def serialize(self, previous):
        ...
        for k, v in six.iteritems(self):
            if k == "id" or (isinstance(k, str) and k.startswith("_")):
                continue
            elif isinstance(v, paayes.api_resources.abstract.APIResource):
                continue
    ...
    def __copy__(self):
        copied = PaayesObject(
            self.get("id"),
            self.api_key,
            paayes_version=self.paayes_version,
            paayes_account=self.paayes_account,
        )
        ...
        for k, v in six.iteritems(self):
            ...
            super(PaayesObject, copied).__setitem__(k, v)
    ...
    def __deepcopy__(self, memo):
        ...
        for k, v in six.iteritems(self):
            # Call parent's __setitem__ to avoid checks that we've added in the
            # overridden version that can throw exceptions.
            super(PaayesObject, copied).__setitem__(k, deepcopy(v, memo))
