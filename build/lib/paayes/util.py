...
from paayes.six.moves.urllib.parse import parse_qsl


PAAYES_LOG = os.environ.get("PAAYES_LOG")

logger = logging.getLogger("paayes")
...
def _console_log_level():
    if paayes.log in ["debug", "info"]:
        return paayes.log
    elif PAAYES_LOG in ["debug", "info"]:
        return PAAYES_LOG
    else:
        return None
...
def _test_or_live_environment():
    if paayes.api_key is None:
        return
    match = re.match(r"sk_(live|test)_", paayes.api_key)
    if match is None:
        return
...
def dashboard_link(request_id):
    return "https://dashboard.paayes.com/{env}/logs/{reqid}".format(
...
def get_object_classes():
    ...
    from paayes.object_classes import OBJECT_CLASSES

    return OBJECT_CLASSES


def convert_to_paayes_object(
    resp, api_key=None, paayes_version=None, paayes_account=None
):
    # If we get a PaayesResponse, we'll want to return a
    # PaayesObject with the last_response field filled out with
    # the raw API response information
    paayes_response = None

    if isinstance(resp, paayes.paayes_response.PaayesResponse):
        paayes_response = resp
        resp = paayes_response.data

    if isinstance(resp, list):
        return [
            convert_to_paayes_object(
                i, api_key, paayes_version, paayes_account
            )
            for i in resp
        ]
    elif isinstance(resp, dict) and not isinstance(
        resp, paayes.paayes_object.PaayesObject
    ):
        resp = resp.copy()
        klass_name = resp.get("object")
        if isinstance(klass_name, six.string_types):
            klass = get_object_classes().get(
                klass_name, paayes.paayes_object.PaayesObject
            )
        else:
            klass = paayes.paayes_object.PaayesObject
...
def convert_to_dict(obj):
    """Converts a PaayesObject back to a regular dict.

    Nested PaayesObjects are also converted back to regular dicts.

    :param obj: The PaayesObject to convert.

    :returns: The PaayesObject as a dict.
    """
    if isinstance(obj, list):
        return [convert_to_dict(i) for i in obj]
    # This works by virtue of the fact that PaayesObjects _are_ dicts. The dict
