...
import paayes
from paayes import error, util, six
from paayes.request_metrics import RequestMetrics
...
try:
    from paayes.six.moves import urllib
except ImportError:
...
try:
    ...
    try:
        ...
        if (major, minor, patch) < (0, 8, 8):
            sys.stderr.write(
                "Warning: the Paayes library requires that your Python "
                '"requests" library be newer than version 0.8.8, but your '
                '"requests" library is version %s. Paayes will fall back to '
                "an alternate HTTP library so everything should work. We "
                'recommend upgrading your "requests" library. If you have any '
                "questions, please contact support@paayes.com. (HINT: running "
...
from paayes.six.moves.urllib.parse import urlparse
...
def new_default_http_client(*args, **kwargs):
    if urlfetch:
        ...
        impl = Urllib2Client
        warnings.warn(
            "Warning: the Paayes library is falling back to urllib2/urllib "
...
class HTTPClient(object):
    ...
    def _should_retry(self, response, api_connection_error, num_retries):
        ...
        if rheaders is not None and "paayes-should-retry" in rheaders:
            if rheaders["paayes-should-retry"] == "false":
                return False
            if rheaders["paayes-should-retry"] == "true":
                return True
    ...
    def _max_network_retries(self):
        from paayes import max_network_retries
    ...
    def _add_telemetry_header(self, headers):
        ...
        if paayes.enable_telemetry and last_request_metrics:
            telemetry = {
                "last_request_metrics": last_request_metrics.payload()
            }
            headers["X-Paayes-Client-Telemetry"] = json.dumps(telemetry)

    def _record_request_metrics(self, response, request_start):
        _, _, rheaders = response
        if "Request-Id" in rheaders and paayes.enable_telemetry:
...
class RequestsClient(HTTPClient):
    ...
    def _request_internal(self, method, url, headers, post_data, is_streaming):
        ...
        if self._verify_ssl_certs:
            kwargs["verify"] = paayes.ca_bundle_path
        else:
            kwargs["verify"] = False
        ...
        try:
            try:
                ...
                raise TypeError(
                    ...
                    '"requests" library is not compatible with Paayes\'s '
    ...
    def _handle_request_error(self, e):
        ...
        if isinstance(e, requests.exceptions.SSLError):
            msg = (
                "Could not verify Paayes's SSL certificate.  Please make "
        ...
        # Retry only timeout and connect errors; similar to urllib3 Retry
            ...
            msg = (
                "Unexpected error communicating with Paayes.  "
                "If this problem persists, let us know at "
                "support@paayes.com."
            )
            err = "%s: %s" % (type(e).__name__, str(e))
...
class UrlFetchClient(HTTPClient):
    ...
    def _handle_request_error(self, e, url):
        if isinstance(e, urlfetch.InvalidURLError):
            msg = (
                "The Paayes library attempted to fetch an "
                "invalid URL (%r). This is likely due to a bug "
                "in the Paayes Python bindings. Please let us know "
                "at support@paayes.com." % (url,)
            )
        elif isinstance(e, urlfetch.DownloadError):
            msg = "There was a problem retrieving data from Paayes."
        elif isinstance(e, urlfetch.ResponseTooLargeError):
            msg = (
                "There was a problem receiving all of your data from "
                "Paayes.  This is likely due to a bug in Paayes. "
                "Please let us know at support@paayes.com."
            )
        else:
            msg = (
                "Unexpected error communicating with Paayes. If this "
                "problem persists, let us know at support@paayes.com."
            )
...
class PycurlClient(HTTPClient):
    ...
    def _request_internal(self, method, url, headers, post_data, is_streaming):
        ...
        if self._verify_ssl_certs:
            self._curl.setopt(pycurl.CAINFO, paayes.ca_bundle_path)
        else:
    ...
    def _handle_request_error(self, e):
        if e.args[0] in [
            ...
            msg = (
                "Could not connect to Paayes.  Please check your "
                "internet connection and try again.  If this problem "
                "persists, you should check Paayes's service status at "
                "https://twitter.com/paayesstatus, or let us know at "
                "support@paayes.com."
            )
            should_retry = True
...
class Urllib2Client(HTTPClient):
    ...
    def _handle_request_error(self, e):
        msg = (
            "Unexpected error communicating with Paayes. "
            "If this problem persists, let us know at support@paayes.com."
        )
        msg = textwrap.fill(msg) + "\n\n(Network error: " + str(e) + ")"
        raise error.APIConnectionError(msg)
